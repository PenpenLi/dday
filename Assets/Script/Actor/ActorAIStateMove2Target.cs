...
public class ActorAIStateMove2Target : ActorAIState 
{
	...
	{
		_timeStamp += dt;
		int nowFrame = (int)(_timeStamp / Battleplayer.TIME_PER_FRAME);
		...
		{
			float lastFrameRemainTime = Battleplayer.TIME_PER_FRAME - _elapsedTimeSinceLastFrame;
			actor.Position = actor.Position + _dir * moveSpeed * lastFrameRemainTime;

			--frameElapsed;

			_elapsedTimeSinceLastFrame = dt - lastFrameRemainTime - frameElapsed * Battleplayer.TIME_PER_FRAME;

			while(frameElapsed > 0)
			{
				_checkDirection();

				actor.Position = actor.Position + _dir * moveSpeed * Battleplayer.TIME_PER_FRAME;
